...
_G.LexusConfig = _G.LexusConfig or {
    ...
    Esp9_Weapon = true, Esp9_Distance = true, Esp9_Line = true, Esp9_Skeleton = true,
    EspAntenna = false, EspOutline = false, OutlineThickness = 10,
    Esp3ShowName = true, Esp3ShowHP = true,
    ColorBodyNew = false, ChamsColorVis = 3, ChamsColorOcc = 1,
    WeaponGlow = false,
...
local function ZN_GetChamsColors(isBot)
    local vi = _G.LexusConfig.ChamsColorVis or 3
    local oc = _G.LexusConfig.ChamsColorOcc or 1
    local LC = getCachedLinearColor()
    local mk = function(r, g, b, a) return (LC and LC(r, g, b, a)) or {R=r, G=g, B=b, A=a} end
    local P = {
        [1] = mk(100, 0, 0, 1),
        [2] = mk(0, 40, 100, 1),
        [3] = mk(0, 100, 0, 1),
    ...
    local vis = P[vi] or P[3]
    local occ = P[oc] or P[1]
    return vis, occ, vis, occ
end
...
local function ApplyColorBodyNew(enemy, markData)
    pcall(function()
        ...
        local stateHash = (isBot and "BOT" or "PLAYER") .. "_" .. tostring(currentMeshCount) .. "_V" .. tostring(_G.LexusConfig.ChamsColorVis or 0) .. "_O" .. tostring(_G.LexusConfig.ChamsColorOcc or 0)
        ...
        local c_vis, c_occ, c_bVis, c_bOcc = ZN_GetChamsColors(isBot)
            ...
            if Valid(mesh) then
                pcall(function()
                    if type(mesh.SetDrawDyeing) == "function" then
                        ...
                        pcall(function()
                            if type(mesh.SetScalarParameterValue) == "function" then
                                mesh:SetScalarParameterValue("invincible", 2.0)
                            end
                            local mm = nil
                            pcall(function() mm = mesh:GetMaterial(0) end)
                            if mm and type(mm.SetScalarParameterValue) == "function" then
                                mm:SetScalarParameterValue("invincible", 2.0)
                            end
                        end)
...
local RedBoxOverlay = {
    ...
    TextBlockCredit = nil, -- MOD BY credit
...
function RedBoxOverlay.Create()
    ...
    -- MOD BY credit text (added by @..... )
    local txtCredit = nil
    pcall(function() txtCredit = CGame:NewObjectFromPath("/Script/UMG.TextBlock", Container) end)
    if txtCredit and slua.isValid(txtCredit) then
        pcall(function()
            txtCredit:SetText("MOD BY :- @..... ")
            RedBoxOverlay._CachedTextCredit = "MOD BY :- @..... "
            local creditColor = FLinearColor and FLinearColor(0.6, 1.0, 1.0, 1.0) or {R=153,G=255,B=255,A=255}
            if FSlateColor then txtCredit:SetColorAndOpacity(FSlateColor(creditColor)) else txtCredit:SetColorAndOpacity(creditColor) end
            if txtCredit.Font then local cf = txtCredit.Font; cf.Size = 12; txtCredit.Font = cf end
            txtCredit:SetRenderScale(FVector2D(RedBoxOverlay.TextScaleValue, RedBoxOverlay.TextScaleValue))
            txtCredit:SetRenderTransformPivot(FVector2D(0.5, 0.5))
            txtCredit:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible)
        end)
        local creditSlot = Container:AddChildToCanvas(txtCredit)
        if creditSlot then
            pcall(function()
                creditSlot:SetAutoSize(true)
                creditSlot:SetAlignment(FVector2D(0.5, 0.5))
                creditSlot:SetPosition(FVector2D(totalWidth * 0.5, RedBoxOverlay.Height + 8))
                creditSlot:SetZOrder(1000)
            end)
        end
        RedBoxOverlay.TextBlockCredit = txtCredit
    end
...
function RedBoxOverlay.Stop()
    ...
    RedBoxOverlay.TextBlockCredit = nil
...
PlayerMapMarker.SnapLineColor = FLinearColor and FLinearColor(1.0, 1.0, 1.0, 1.0) or {R=255, G=255, B=255, A=255} -- TRẮNG
        ...
        if SizeBox_HP and slua.isValid(SizeBox_HP) then
            ...
            if ExistingChild and slua.isValid(ExistingChild) then
                ...
                if FoundPB and slua.isValid(FoundPB) then
                    ...
                    if PB then
                        pcall(function() PB:SetFillColorAndOpacity(FLinearColor and FLinearColor(0.88, 0.96, 1.0, 1.0) or {R=224,G=245,B=255,A=1}) end)
        ...
        if bValid then
            ...
            pcall(function()
                ...
                if Widget.HealthFill.SetPercent then
                    ...
                    -- WATER WHITE HP BAR
                    local color = FLinearColor and FLinearColor(0.88, 0.96, 1.0, 1.0) or {R=224,G=245,B=255,A=255}
...
function PlayerMapMarker.UpdateSkeletonLines(KeyStr, Character, PC, bVisible, TeamColor, bPlayerOnScreen, charLoc)
    ...
    if PlayerMapMarker.bUseVisibilityColor then
        ...
        if bTargetVisible then lineColor = FLinearColor(1.0, 1.0, 1.0, 0.9)
        else lineColor = FLinearColor(1.0, 1.0, 1.0, 0.9) end
    else
                        ...
                        pcall(function()
                            if Valid(Cached_MyHUD) and distM <= 400 then
                                ...
                                if _G.LexusConfig.Esp3ShowName then
                                    ...
                                    Cached_MyHUD:AddDebugText(enemyName, enemy, 0.06, {X=0, Y=0, Z=-370}, {X=0, Y=0, Z=-370}, C_WHITE, true, false, true, nil, dynamicScale * 1.1, true)
                                end
...
local ConfigFileName = "Zn_Knox_settings.txt"
...
function _G.InitModMenuTab()
    ...
    if not SettingPageDefine.ModMenu then
        ...
        local StackESP = {
            ...
            { Key = "ModMenu_ESP9_Skeleton", UI = AliasMap.Switcher, Text = T("   Show Skeleton (may lag)"), ExpandHandle = "ModMenu_ESP9_Ex", GetFunc = function() return _G.LexusConfig.Esp9_Skeleton end, SetFunc = function(c,v) _G.LexusConfig.Esp9_Skeleton = v return true end },
            ...
            { UI = AliasMap.Title, Text = T("   === CHAMS FRONT (OPEN) ===") },
            { Key = "ModMenu_ChamsVisColor", UI = AliasMap.Switcher, Text = T("FRONT COLOR"),
              SwitcherText = {"Green","Red","Yellow","Pink","Orange","Blue"},
              SwitcherValue = {3,1,4,6,5,2},
              GetFunc = function() return _G.LexusConfig.ChamsColorVis or 3 end,
              SetFunc = function(_, v) _G.LexusConfig.ChamsColorVis = v; return true end },
            { UI = AliasMap.Title, Text = T("   === CHAMS THROUGH WALL ===") },
            { Key = "ModMenu_ChamsWallColor", UI = AliasMap.Switcher, Text = T("WALL COLOR"),
              SwitcherText = {"Green","Red","Yellow","Pink","Orange","Blue"},
              SwitcherValue = {3,1,4,6,5,2},
              GetFunc = function() return _G.LexusConfig.ChamsColorOcc or 1 end,
              SetFunc = function(_, v) _G.LexusConfig.ChamsColorOcc = v; return true end },
        }
...
local function ShowLexusVIPMenu()
    ...
    pcall(function()
        ...
        local function Step_ScamAlert()
            local title = "Zn_Knox PAK FILE"
            local content = "Zn_Knox MOD\n=======================\nTELEGRAM - @..... \nOWNER - @..... \n======================="
            local btn1 = "JOIN"
            local btn2 = "CLOSE"
            Msg.Show(1, title, content, function() local Web = require("client.slua.logic.url.logic_webview_sdk"); if Web and Web.OpenURL then Web:OpenURL("@..... ") end end, function() end, btn1, btn2)
...
-- ===== DASNY/FUFUMOD BYPASS MERGE START (expiry 12 Aug 2026) =====
do
local limitTime = os.time({ year = 2026, month = 9, day = 12, hour = 23, min = 59, sec = 0 })
...
_G.StartBypass_VIP_v3 = function()
    if _G.DASNY_BYPASS_DONE then return end
    _G.DASNY_BYPASS_DONE = true
    if isExpired then print("[DASNY] bypass expired (12 Aug 2026)") return end
    pcall(function()
        print("[NİHAİ BYPASS] Başlatma işlemine başlanıyor...")
...
-- ===== DASNY/FUFUMOD BYPASS MERGE END =====

-- ===== KILL BROADCAST SKIN INJECT HOOK (by X) =====
if not _G.ZN_KillBroadcastHacked then
    pcall(function()
        local BattleKillBroadcastSubSystem = require("GameLua.Mod.BaseMod.Client.BattleKillBroadcast.BattleKillBroadcastSubSystem")
        if BattleKillBroadcastSubSystem and not _G.ZN_KillBroadcastHacked then
            ...
            _G.ZN_KillBroadcastHacked = true
        end
    end)
end
-- ===== KILL BROADCAST SKIN INJECT HOOK END =====
